...
async function searchCineby(title) {
  ...
  try {
    const resp = await axios.get(`https://www.cineby.app/search?query=${encodeURIComponent(title)}`, { timeout: 7000 });
...
async function getMovieStreams(title) {
  ...
  if (!moviePage) return [];
  try {
    const resp = await axios.get(moviePage, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 7000 });
    const $ = cheerio.load(resp.data);
    const streams = [];
...
async function getSeriesEpisodes(title) {
  ...
  try {
    const resp = await axios.get(seriesPage, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 7000 });
    const $ = cheerio.load(resp.data);
    const episodesList = [];
    $('[id^=season-]').each((s, seasonEl) => {
      ...
      $(seasonEl).find('.episode').each((e, epEl) => {
        ...
        const subs = [];
        $(epEl).find('track[kind="subtitles"]').each((i, t) => subs.push({ url: $(t).attr('src'), lang: $(t).attr('srclang') || 'en' }));
        if (iframe) {
          episodesList.push({ season: seasonNum, episode: episodeNum, streams: [{ title: 'Cineby', url: iframe, lang: 'en', rel: ['native'], subtitles: subs }] });
        }
      });
    });
...
module.exports = async (req, res) => {
  try {
    ...
    res.status(500).send('Addon server error');
  }
};
